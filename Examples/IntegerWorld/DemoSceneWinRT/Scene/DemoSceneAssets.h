...
namespace Assets
{
	...
	namespace Shaders
	{
		...
		{
			light_source_t* LightSource = nullptr;
			...
			void FragmentShade(WindowRasterizer& rasterizer, const billboard_fragment_t& fragment) final
			{
				...
				for (int_fast16_t y = 0; y < radius; y++)
				{
					for (int_fast16_t x = 0; x < radius; x++)
					{
						...
						{
							const uint8_t innerComponent = Fraction(proximityFraction, Rgb8::COMPONENT_MAX);
							color = Rgb8::Color(
								Fraction(proximityFraction, uint8_t(UINT8_MAX)),
								uint8_t(Fraction(distanceFraction, Rgb8::Red(lightColor)) + innerComponent),
								uint8_t(Fraction(distanceFraction, Rgb8::Green(lightColor)) + innerComponent),
								uint8_t(Fraction(distanceFraction, Rgb8::Blue(lightColor)) + innerComponent));

							// Blend mode Add looks better but Alpha still looks ok in monochrome screens.
		...
		struct FloorFragmentShader : IFragmentShader<point_normal_fragment_t>
		{
		public:
	...
	namespace Objects
	{
		...
		struct StarMeshObject : public StaticMeshSingleColorSingleMaterialObject<Star::VertexCount, Star::TriangleCount>
		{
			StarMeshObject() : StaticMeshSingleColorSingleMaterialObject<Star::VertexCount, Star::TriangleCount>(
				Star::Vertices,
				Star::Triangles) {
			}
		};

		struct CubeMeshObject : public StaticMeshObject<Cube::VertexCount, Cube::TriangleCount>
		{
			material_t Material{ 0, UFRACTION8_1X / 2, 0, 0 };

			CubeMeshObject() : StaticMeshObject<Cube::VertexCount, Cube::TriangleCount>(
				Cube::Vertices,
				Cube::Triangles) {
			}
		...
		class SphereMeshLodObject : public StaticMeshLodSingleColorSingleMaterialObject<Assets::Shapes::SphereLod::LoD0::VertexCount,
			Assets::Shapes::SphereLod::LoD0::TriangleCount, 2>
		{
		private:
		...
		struct FloorPointCloudObject : FlatPointCloudObject<Grid8x8::VertexCount>
		{
			FloorPointCloudObject()
				: FlatPointCloudObject<Grid8x8::VertexCount>(Grid8x8::Vertices)
			{
			}
		};
	}
